...
class ClothSim {
  constructor(numX, numY){
    ...
    // Multi-touch: several particles can be grabbed at once (one per
    // finger/pointer), each remembering its own ray depth so two hands can
    // pinch, stretch, and drag different points of the same piece.
    this.grabbed = new Map(); // idx -> grabDepth
  }

  isGrabbed(idx){ return this.grabbed.has(idx); }

  /** (Re)lay out particles for a given hang style. Recomputes rest lengths. */
  reset(layout){
    const { width, height, offsetY, pinMode, centerX } = layout;
    this.width = width;
    this.height = height;
    const cx = centerX || 0;
    this.grabbed.clear();
    ...
    for(let y=0;y<this.numY;y++){
      for(let x=0;x<this.numX;x++){
        const px = (x/(this.numX-1)-0.5) * width + cx;
  ...
  step(opts){
    ...
    for(let i=0;i<this.numParticles;i++){
      if(this.pinned[i] || this.grabbed.has(i)) continue;
    ...
    for(let iter=0; iter<iterations; iter++){
      for(let i=0;i<this.constraints.length;i++){
        ...
        const w1 = (this.pinned[c.p1] || this.grabbed.has(c.p1)) ? 0 : 1;
        const w2 = (this.pinned[c.p2] || this.grabbed.has(c.p2)) ? 0 : 1;
        const wSum = w1+w2;
        if(wSum>0){
  ...
  /** Ray-pick the nearest not-already-grabbed particle within `maxDist`,
   *  returns index or -1. Skipping grabbed particles means a second (or
   *  third) simultaneous touch always finds a fresh point to hold, instead
   *  of fighting the first finger for the same vertex. */
  pick(ray, maxDist){
    ...
    if(bestIdx!==-1) this.grabbed.set(bestIdx, bestT);
    return bestIdx;
  }

  dragTo(idx, ray){
    if(!this.grabbed.has(idx)) return;
    const depth = this.grabbed.get(idx);
    const p = this.particles[idx];
    p.x = ray.origin.x + ray.dir.x*depth;
    p.y = ray.origin.y + ray.dir.y*depth;
    p.z = ray.origin.z + ray.dir.z*depth;
    p.ox=p.x; p.oy=p.y; p.oz=p.z;
  }

  /** Gather mode: drag particle + pull nearby particles inward */
  gatherTo(idx, ray, radius){
    if(!this.grabbed.has(idx)) return;
    this.dragTo(idx, ray);
    const target = this.particles[idx];
    const rSq = radius*radius;
    for(let i=0;i<this.numParticles;i++){
      if(i===idx || this.pinned[i] || this.grabbed.has(i)) continue;
      const p = this.particles[i];
      const dx=p.x-target.x, dy=p.y-target.y, dz=p.z-target.z;
      const dSq = dx*dx+dy*dy+dz*dz;
      if(dSq < rSq){
        const t = 1 - Math.sqrt(dSq)/radius;
        const pull = t*t * 0.5;
  ...
  /** Fold/crease: pin a particle relative to another, creating a fold line */
  foldTo(idx, targetIdx){
    if(this.grabbed.has(idx) || this.pinned[idx]) return;
    const p = this.particles[idx];
    const t = this.particles[targetIdx];
    p.x = t.x + (p.x-t.x)*0.5;
    p.y = t.y + (p.y-t.y)*0.5;
    p.z = t.z + (p.z-t.z)*0.5;
    p.ox=p.x; p.oy=p.y; p.oz=p.z;
    this.pinned[idx] = true;
    this._foldPairs = this._foldPairs || [];
    this._foldPairs.push({ a:idx, b:targetIdx, rest:0.5 });
    // add a short constraint between the points to hold the fold
    this.constraints.push({ p1:idx, p2:targetIdx, rest:0.5, isFold:true });
  }
  ...
  /** Remove any fold constraints created by folding */
  releaseFolds(){
    if(!this._foldPairs) return;
    this.constraints = this.constraints.filter(c => !c.isFold);
    this._foldPairs.forEach(fp => { this.pinned[fp.a] = false; });
    this._foldPairs = null;
  }
}
